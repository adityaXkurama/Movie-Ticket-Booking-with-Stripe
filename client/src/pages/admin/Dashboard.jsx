...
const Dashboard = () => {
  ...
  return !loading ?(
    < >
      ...
      <div className="relative flex mt-6 gap-4 flex-wrap">
        <BlurCircle top='-100px' left='0'/>
        <div className='flex flex-wrap gap-4 w-full'>
          {
          dashboardCards.map((card,index)=>(
            <div key={index} className='flex items-center justify-between px-4 py-3 bg-primary/10 border border-primary/20 rounded-md max-w-52 w-full'>
              <div>
                <h1 className='text-sm'>{card.title}</h1>
                <p className='text-xl mt-1 font-medium'>{card.value}</p>
              </div>
      ...
      <div className='relative flex flex-wrap gap-6 mt-4 max-w-5xl'>
        <BlurCircle top='100px' left='-10%'/>
        {
          dashboardData.activeShows.map((show)=>(
            <div key={show._id} className='w-56 flex flex-col gap-2 rounded-xl bg-primary/15 hover:-translate-y-1 transition duration-300 h-full border border-primary/25 pb-3'>
